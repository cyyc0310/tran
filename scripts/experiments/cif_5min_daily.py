...
# 上海 2023 真实口径 (上海电力供应环境可持续性 KPI 报告 2023, 沪经信运
# 〔2024〕306 号): 年用电 1849 亿 kWh, 本市发电 1015 亿 (54.9%), 市外来电
# 834 亿 (45.1%)。本地结构 (亿 kWh): 火电 926 (煤 ~696 + 气 ~225 + 油 ~5,
# 官方火电 926.4 北极星规上口径印证) + 风光 48 (官方新能源发电量 48 亿)
# + 生物质 ~41 (官方可再生 89 亿 = 风光 48 + 生物质 41, 垃圾焚烧为主)。
# 可再生占本地发电 8.8% — 与官方 KPI 完全一致。本地无核电无水电 (秦山
# 在浙江, 计入受电)。月度分布为典型形态推断 (冬夏双峰 + 光伏季节角),
# 无逐月公开数据, 诚实标注。单位仅用于份额计算 (相对量)。
SH_SOLAR_12 = [0.9, 1.2, 1.9, 2.6, 3.1, 3.4, 3.3, 3.0, 2.3, 1.7, 1.1, 0.9]
SH_WIND_12 = [1.9, 2.0, 2.2, 2.2, 2.0, 1.8, 1.6, 1.6, 1.9, 2.1, 2.1, 1.9]
SH_COAL_12 = [65, 59, 61, 56, 53, 55, 57, 56, 54, 56, 60, 64]
SH_GAS_12 = [19, 17, 18, 17, 16, 20, 22, 22, 19, 17, 18, 20]
SHANGHAI_GEN_2023 = {
    "coal": SH_COAL_12, "gas": SH_GAS_12,
    "petroleum": [0.4] * 12, "nuclear": [0.0] * 12, "hydro": [0.0] * 12,
    "solar": SH_SOLAR_12, "wind": SH_WIND_12, "biomass": [3.4] * 12,
}
SHANGHAI_THERMAL_EFS = {"coal": 448.0, "gas": 490.0, "petroleum": 720.0}

# 上海 2023 市外来电 (亿 kWh/月, 年总 834 锚定官方): 最大受电在夏季
# (2023 最高受电 1812 万 kW, 2024-07 达 2089 万 kW), 冬季寒潮双高
# (2023-12 强寒潮冬峰负荷 3499 万 kW 创历史新高), 春秋相对低。月度形态
# 按送端水电季节性 (西南丰枯) + 受端负荷形态推断, 总量精确锚定 834。
SHANGHAI_IMPORTS_2023 = [74, 65, 60, 58, 62, 70, 80, 78, 72, 68, 72, 75]

# 送端三类结构 (flow-weighted monthly imports EF, 12 月份额):
#   XN_hydro   西南水电: 葛南/宜华/林枫 (三峡+葛洲坝) + 复奉 (向家坝,
#              金沙江) 四条直流 2023 合计 479 亿 kWh (>受电一半; 东吴
#              证券引全国电力可靠性报告)。直流专送水电纯度高; 枯水期
#              送端电网火电顶替反映为 CIF 轻微抬升 (冬 25 → 夏 8)。
#   AH_coal    皖电东送煤电: 两淮煤电基地经特高压交流入沪; 2023 皖电
#              东送沪苏浙 683 亿 (安徽省政府), 沪份额 ~240 亿; CIF 按
#              安徽煤电 ~862 (含少量网内苏浙火电交流)。
#   QN_nuclear 秦山核电 + 绿电交易 (21.5 亿, 官方) + 网内清洁: 近零碳。
SHANGHAI_SENDER_CIF = {
    "XN_hydro":   [25, 25, 25, 20, 15, 10, 8, 8, 12, 18, 22, 25],
    "AH_coal":    [880, 875, 870, 865, 860, 855, 850, 850, 860, 870, 875, 880],
    "QN_nuclear": [15] * 12,
}
SHANGHAI_FLOW_SHARES = {  # 月度份额 (行和=1): 水电 479 / 皖煤 ~240 / 核绿 ~118
...
def build_forecaster(province, legacy_lvl=False, seed=0, epochs=600, device="mps"):
    ...
    def predict_day(target_date, variant=None, weather_fn=None,
        ...
        weather_fn    : 可选天气注入 (hours, prov) -> (T,3)，默认合成气候
                        部署时替换为 day-ahead NWP; 压测时注入极端预报。
        inter_monthly : 可选 (12,) 受电份额覆盖 (电网断裂场景注入)。
        base_weather_fn: 可选正常日天气基线 (消融: synthetic vs ERA5)。
                        None = synth_weather_year (与历史默认一致)。
        """
        y = target_date.year
        if y == 2023:
...
def eval_against_daily_truth(predict_day, meta, year, months=None,
    ...
    weather_mode: "synthetic" (历史默认) | "era5" (真实再分析天气消融)。
    CM 日度 value 为 Mt/day。真值日 CIF = 日排放 / 日发电量, 但无逐日
    发电; 用月度发电 / 天数做分母 (月内日均), 与月度真值同构。日评估
    因此检验的是「日更产品的日均水平」落在月真值水平附近 + CM 日度
    排放波动的捕捉 (经由形状—排放联动此处不建模, 诚实说明)。
    """
    prov = meta["province"]
...
def run_stress_test(predict_day, meta, target_date, base_weather_fn=None):
    ...
    base_weather_fn: 正常日基线天气 (None = 合成气候; 消融可传 ERA5)。
    极端场景 = 基线 + horizon 内扰动注入, 与基线选择解耦。
    """
    prov = meta["province"]
...
def main():
    ...
    ap.add_argument("--stress", action="store_true",
                    help="极端天气/电网断裂压力测试")
    ap.add_argument("--weather", default="synthetic",
                    choices=["synthetic", "era5"],
                    help="正常日天气基线: synthetic (默认, 合成气候) | "
                         "era5 (真实再分析, 2023-2024 太原/上海)")
