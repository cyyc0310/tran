...
Every state-of-the-art grid carbon intensity (CIF) forecaster is trained on months of local data from a single region — onboarding a new grid means collecting emissions telemetry, retraining, and revalidating. We ask: **can a model trained on one set of electricity grids forecast a never-seen grid in a different country?** We formalize **config-only zero-shot cross-region CIF prediction**: TransCIF is trained on source regions and deployed to a target given only two public configuration scalars (mean renewable share $\bar{rs}$, non-renewable emission factor $ef_{nr}$) and the real-time renewable-share stream. A physics decomposition layer reconstructs CIF exactly from the predicted share via $\mathrm{CIF} = rs\cdot ef_r + (1-rs)\cdot ef_{nr}$, making the learning target transferable across grids while region-specific scale is absorbed by a lookup table.

On a 29-region, 3-jurisdiction benchmark (AU NEM, UK DNO, US EIA-930; 5 seeds), cross-domain transfer works: **training exclusively on US+UK data, TransCIF forecasts all 4 Australian grids — in a different hemisphere, market, and fuel regime — and beats persistence on each** (QLD1: 27.0 MAE, surpassing the supervised PatchTST's 30.3). Across all 29 regions, the zero-shot model reaches median ratio $\rho=1.24$ vs. supervised PatchTST; a training-free test-time calibration ($\mathcal{I}_+$, still zero future labels) tightens this to $\rho^+=1.08$ and beats persistence in **29/29 regions**; a differentiable five-prior fusion with 12 days of calibration labels reaches median MAE **39.53** ($\rho^J=0.95$, 61% pairwise wins vs. PatchTST 41.47). The equalizer effect — four architecturally diverse backbones converge to within 0.05 MAE of the same calibrated median — shows competitiveness comes from physics + config + calibration combined.

The physics layer yields an exact error identity (Theorem 1, verified to $1.3\times10^{-4}$; $R^2=0.84$) and a config-space transfer law (Theorem 2, U-shaped in renewable penetration, $R^2=0.66$) that predicts deployment difficulty before any forecast is issued. Against CarbonCast (the leading supervised method), TransCIF wins 8/9 regions zero-shot (mean ratio 0.73) while CarbonCast degrades 2.4× cross-domain. Conformal intervals achieve valid 90% coverage in 25/29 regions; temporal-OOD splits leave $\mathcal{I}_+$ ratio flat at 0.82; and supervised models need 30–270+ days to match day-one zero-shot accuracy.
...
**Contributions.**
1. **Cross-jurisdictional transfer works.** We demonstrate that a model trained on US+UK grids forecasts Australian NEM regions — different hemisphere, market structure, and fuel regime — and beats persistence on every one (§6.3). A leave-one-jurisdiction-out experiment confirms the effect: excluding all US sources, the model still forecasts US targets from AU+UK data alone (§6.3).
2. **Problem formalization and open benchmark.** We formalize config-only zero-shot cross-region CIF prediction and release a 29-region, 3-jurisdiction LORO benchmark ($\bar{rs}\in[0.06, 0.91]$, 5 seeds).
3. **Physics-decomposed architecture.** TransCIF couples a config-conditioned share predictor with a fixed linear physics layer, yielding an exact error identity (Theorem 1, verified to $1.3\times10^{-4}$, $R^2=0.84$) and a config-space transfer law (Theorem 2, U-shaped in penetration, $R^2=0.66$).
4. **Information-set tiers and the equalizer effect.** Zero-shot ($\rho=1.24$) → test-time calibration ($\rho^+=1.08$, 29/29 beat persistence) → joint calibration (MAE 39.53, $\rho^J=0.95$). Four architecturally diverse backbones converge to within 0.05 MAE of the same calibrated median — competitiveness comes from physics + config + calibration, not architecture.
5. **Comprehensive empirical case.** 8/9 wins over zero-shot CarbonCast (mean ratio 0.73; it degrades 2.4× cross-domain); conformal intervals in 25/29 regions; temporal-OOD robustness; supervised models need 30–270+ days to match day-one zero-shot accuracy.

We report negative and mixed results alongside positive ones: TransCIF loses to persistence in fossil-dominated, near-constant-CIF regions (where persistence is nearly optimal for *every* method), and CarbonCast remains stronger when the target's distribution happens to resemble the source pool. The claim is not universal dominance; it is that **cross-region transfer buys ~80–95% of supervised accuracy predictably, with calibrated uncertainty, and the transfer difficulty is computable from two public numbers before deployment.**
...
<p align="center">
  <img src="figures/fusion_weights.png" alt="direction contributions" width="46%">
  <img src="figures/joint_dropone_ablation.png" alt="drop-one ablation" width="46%">
</p>
<p align="center"><em>Left: the five direction priors span complementary strengths (causal strongest at 41.2 zero-shot; hierarchical weakest at 71.1 but rescued to 46.6 by ZS+). Right: drop-one ablation — removing any direction leaves the fused MAE essentially flat, confirming the learned meta-learner adds no value over equal weighting at pure zero-shot.</em></p>
...
| Joint, all-5-native + int-val gate | **39.04** | + RAG/ICL native + overfit gate |

The frozen-proxy stage (40.53) is the Joint row of Table 1; making the direction models torch-native improves it to **39.53** (three live directions) and **39.04** (all five native plus a conservative internal-validation gate that reverts Stage-2 head finetuning when it overfits a disjoint inner-validation split on easy grids) — the first sub-40 system median — by giving the calibration a true gradient path into each prior that the fixed correction proxy captured only partially. The 39.04 variant is the recommended production config: it is the first to clear statistical significance against the supervised PatchTST reference (one-sample Wilcoxon $p=0.045$, 61% pair-wise win), a bar the 39.53 variant only approaches ($p=0.057$); we note the 39.04 vs 39.53 difference is itself within noise (paired $p=0.12$), so the defensible claim is that *torch-native joint calibration as a class* significantly beats the frozen proxy.

<p align="center">
  <img src="figures/joint_mae_progression.png" alt="MAE progression" width="72%">
</p>
...
<p align="center"><em>Figure: per-region torch-native MAE vs frozen-proxy MAE. Points below the diagonal are regions the torch-native model improved; the largest gains (VIC1, UK_08, UK_09) are exactly the hard-tier grids stuck near their persistence floor, while a few easy grids regress slightly (above the diagonal) — caught by the internal-validation gate.</em></p>
...
|---|---|---|
| 29-region LORO benchmark (Table 1) | `scripts/benchmark/run_unified_eval.py` | `results/unified_eval_full.json` |
| Supervised baseline suite (§6.3) | `scripts/experiments/run_phase1_complete.py` | `docs/experiments/2026-07-25-supervised-baselines-results.md` |
