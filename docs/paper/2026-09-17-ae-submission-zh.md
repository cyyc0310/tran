...
**目标期刊**：*Applied Energy*（research paper）。本稿为英文投稿稿 `2026-09-17-ae-submission.md` 的中文平行版本，结构、口径与数字保持一致；全部数值可追溯至可复现性说明所列归档结果文件。

---

## Highlights

- 构建严格嵌套的信息层级体系，逐层量化从零遥测到全监督各部署场景下碳强度预测的信息价值。
- 参数量约 2.1 万的物理燃料分解网络在 29 个真实区域中的 18 个上优于经 10 个月本地数据训练的监督模型。
- 仅以美国与英国区域为训练源，模型即在 4 个澳大利亚电网中的 3 个上优于本地监督基线。
- 基于纯天气指标的可复现事件日评估协议表明，Dunkelflaute（风-光同枯）复合事件日的预测误差系统性放大 2 至 4 倍。
- 部署翻译层使同一模型仅依赖公开月度统计数据即可应用于中国省级电网，月聚合 MAE 为 15–18 gCO₂/kWh。

---

## 摘要

逐时电网碳强度（carbon intensity factor, CIF，单位 gCO₂/kWh）预测是碳感知负载调度的前提，但现有预测模型均要求目标电网具备数月的燃料遥测数据并完成本地重训练，这恰恰将碳感知调度边际价值最高的地区排除在外。本文将零遥测碳强度预测形式化为严格嵌套信息集的层级体系——仅公开配置标量 → 实时燃料份额流 → CIF 历史 → 滞后观测 → 全监督——并在统一协议下对三个电力市场辖区（AEMO、National Grid ESO、EIA-930）的 29 个真实区域逐层评估。零遥测层由参数量约 2.1 万的燃料分解网络实现：模型不直接回归碳强度，而是预测十条燃料份额轨迹并经查表排放因子线性重构，排放因子作为常数不参与学习；各燃料预测头均注入燃料特定的物理归纳偏置（光伏为天气调制的确定性天文包络，风电为带风旱锚定的容量因子比值结构，基荷为掩码水平统计，火电为配置先验初始化的可调度残差），全部区域特定尺度信息由查表排放因子承担。零参数结构路由将高风电与水电主导的配置分配至聚合路径。在 145 次留一区域实验（5 个随机种子）中，零遥测层的中位 MAE 为 38.4 gCO₂/kWh，低于在目标区域 10 个月数据上训练的 PatchTST（43.5）与 CarbonCast（43.4），并在 29 个区域中的 18 个上直接占优；遥测层中位 MAE 为 36.9（17/29 区域占优）。在留一辖区迁移设定下（仅以 US+UK 为训练源），模型在 4 个澳大利亚电网中的 3 个上优于本地监督基线。本文进一步量化了基于再分析天气的学术评估通常忽略的部署代价：以业务数值天气预报替换再分析天气使合并 MAE 恶化 6.4 gCO₂/kWh；天气定义的 Dunkelflaute 复合事件日误差放大 2 至 4 倍（348 项方法-区域检验中 62 项显著）；20 gCO₂/kWh 的零遥测精度目标经证明不可达（水平替换实验给出的误差下界为 23.1）。部署翻译层使同一模型仅依赖公开月度统计数据即可应用于中国省级电网，在山西与上海的独立真值回测中月聚合 MAE 为 15–18 gCO₂/kWh。该层级体系与中国分级披露制度天然适配：信息披露深化至哪一层级，方法即可相应升级至该层级。

**关键词**：碳强度预测；零样本迁移；物理信息神经网络；可再生能源；Dunkelflaute（风-光同枯）；中国电网

---

## 1. 引言

电力系统脱碳要求准确掌握电网碳强度的时变特征。逐时碳强度预测支撑碳感知负载调度、电动汽车有序充电与需求响应等应用。过去十年的研究已发展出精度可观的区域内预测模型——从层次化 CNN-LSTM 系统 [1] 到小波-块架构 [3]——但均遵循"一区域一模型"范式：接入新电网意味着汇集燃料遥测、重新训练与重新验证。全球多数电网难以满足这一数据前提。以中国为例：七个正式运行的省级现货市场已发布 15 分钟级分电源调度数据，但获取需市场成员资格，各省数据格式不一、历史积累有限，平衡区口径不含自备电厂，且官方碳强度仅为滞后约两年的年度均值。碳感知调度边际价值最高的地区，恰恰是前置数据最难获得的地区。

针对这一矛盾，本文研究如下边界设定：在目标区域零遥测条件下，新电网的碳强度预测可达到何种精度？本文将问题组织为严格嵌套信息集的层级体系：
...
零遥测层的技术实现为燃料分解网络（FuelDecompNet）。模型不直接回归碳强度：网络预测十条燃料份额轨迹，物理层经固定查表排放因子重构 CIF，排放因子不参与学习。各燃料头注入燃料特定的归纳偏置：光伏出力由纯天文通道主导；风电出力遵循锚定于风旱稳定参考水平的容量因子比值结构；基荷燃料服从支撑掩码约束（月度配置表中不存在的燃料不能产生幻觉出力）；火电出力为可调度残差，以配置先验初始化。设计遵循"水平-形态结构分离"原则：绝对水平（排放因子、平均份额）为区域特定量，由查表与配置信息承担；形态（日内动态、天气响应）是唯一被学习、因而可迁移的成分。零参数结构路由——配置向量的确定性函数——将高风电与水电主导区域分配至聚合路径（其形态由观测的可再生份额流承载），其余区域分配至分解路径。

本方法的三项构成要素支撑了架构的可部署性，且每一项本身即构成贡献：

1. **统一 UTC 时基的输入管道。**本方法要求全部输入在天文与日历特征计算前统一对齐至 UTC 时基。该规范并非工程细节：时区错位配置下，天文通道与光伏份额的相关系数由 +0.976 退化为 −0.697，而天文通道本身为确定性物理量，此类缺陷无法由任何架构层面的改进弥补。
2. **场站加权天气。**以装机容量加权的场群天气替换质心网格天气，使风电份额的解释方差由 R² 0.08–0.15 提升至 0.33–0.48。该增益为全部消融中幅度最大的单项改进，且源于数据工程而非模型架构。
3. **容量约束。**全模型参数量约 2.1 万。消融实验中，深度条件化机制（特征调制、超网络、元学习融合）在零样本迁移设定下一致降低性能：高容量编码器会记忆源区域的时序惯用模式，而该模式不可迁移。零遥测碳强度预测中存在的负容量缩放现象本身即为一项发现。

**贡献。**

1. **跨域碳强度预测的信息层级形式化与逐层评估。**在 29 区域 × 5 随机种子的 LORO 实验中，零遥测层（中位 MAE 38.4）优于以目标区域 10 个月数据训练的 PatchTST（43.5）与 CarbonCast（43.4），并在 18/29 区域直接占优——首次在总体层面证明零遥测方法可优于 10 个月监督训练。
2. **物理分解架构及精确误差传播恒等式。**定理 1 将 CIF 预测误差分解为物理常数加权的份额误差与可测物理残差之和，该分解与具体架构无关，可给出逐燃料误差归因与无需训练代价的区域难度排序。命题 1（实证）刻画零遥测方法优于监督的条件：监督模型继承随排放因子对比度放大的标签噪声，零遥测方法承担随物理通道质量与配置覆盖收缩的迁移残差；观测到的区域胜负格局遵循该结构。
3. **可复现事件日评估协议及其揭示的部署代价。**纯天气、可由再分析数据复现的 Dunkelflaute 标签揭示 2–4 倍的系统性误差放大（348 项方法-区域检验中 62 项显著，最大 4.19 倍），且集中于苏格兰-北英格兰地带。以业务数值天气预报替换再分析使合并 MAE 恶化 6.4 gCO₂/kWh。20 gCO₂/kWh 的零遥测目标经证明不可达（水平误差下界 23.1）。上述代价应作为碳感知系统设计的关键输入，而非次要注释。
4. **面向中国的部署翻译层。**月度公开统计数据经四组件翻译（中国日历重映射、送端加权进口排放因子、月度配置表、代理锚定信任门控）转换为模型输入契约，在山西与上海的独立真值回测中月聚合 MAE 达 15–18 gCO₂/kWh。该层级体系与中国分级披露制度相适配：当披露由月度统计深化至市场成员燃料数据时，同一模型无需工程改动即可升级层级。

**主张的边界。**本文不主张零遥测方法普遍优于监督方法。在燃料标签由分类器合成的 SA1 区域，本地监督模型保持明显优势（跨辖区迁移设定下 65.2 对 69.3）；NSW1 区域的监督模型同样受益于物理路径不可见的标签特异性信息。事件日误差与右尾误差存在系统性放大，业务天气预报带来已量化的精度损失，中国部署中日内形态幅度被低估 2–3 倍。本文的主张更为具体也更具实用价值：在遥测缺失或不可得的全部场景中，零遥测层给出显著优于无预测与持久性基线的预测，通常与 10 个月监督训练持平或更优；且该结论可在任何部署决策之前基于公开数据预判。
...
**碳强度预测。**CarbonCast [1] 是该方向的代表性系统：两级 CNN-LSTM 先预测分电源出力再合成 CIF，逐区域本地训练，在六个区域上取得 4.8–13.9% 的多日 MAPE。DACF [2] 确立了日前预测的问题形式。近期架构持续改进区域内精度——Zhang 等 [3] 的小波-块模型在四个澳大利亚市场上捕捉跨频率的局部时序与跨变量依赖——但均保留一区域一模型范式。本文在两个角色上使用 CarbonCast：其一为监督参考上界（本文的忠实复现在相同测试窗口上训练，中位 MAE 43.4，与 PatchTST 的 43.5 相当）；其二为跨域基线（该角色超出其原始设计假设）。

**数据稀缺与跨区域碳强度预测。**DGCFM [4] 通过预训练时序基础模型、元数据驱动超图微调与时空碳图应对数据稀缺问题，报告低数据场景下约 20% 的精度提升，但目标域微调仍为其必要成分。CarbonX [5] 完全去除燃料结构依赖，仅凭历史序列以时序基础模型预测 CIF（在 214 个电网上零样本 MAPE 15.8%），并将天气协变量列为未来工作。本文设定在一个方向上更为严格（基础层完全不含 CIF 历史），在另一方向上更为丰富（允许公开配置与天气信息）；实验结果表明该信息契约值得其数据成本。本文的解决方案是结构性的（物理层承担尺度）而非规模驱动的（基础模型容量）。

**时序基础模型。**Chronos-2 [6] 通过组注意力的上下文学习支持零样本单变量、多变量与协变量感知预测。本文不将 Chronos-2 用作端到端替代，而是作为滞后层（§4.5）内的形态先验：其贡献以 EWMA 水平校正与持久性锚定为前提，对其输出再次调整水平反而有害。这一分工——基础模型负责形态、经典组件负责水平——本身构成一项发现。

**Dunkelflaute 与事件日分析。**风能与太阳能同时低可用期是能源系统研究的活跃议题；Kittel 与 Schill [11] 记录了现有定义在四个维度（产量参数、持续时间、空间范围、阈值类型）上的分歧，尚无标准定义形成。本文协议（§8.4）给出面向预测评估的实例化：逐区域逐年 20 分位阈值作用于再分析派生的日均风速与太阳辐射，复合事件要求二者同时满足，暗冬区域仅评估低风日；标签纯由天气派生，任何团队均可复现，且应用于预测误差分桶而非资源评估。

**域泛化。**经典理论 [10] 以源域风险与域间散度界定目标域风险。本文定理 1 将散度项替换为可观测的领域特定量——排放因子对比度。此外，§8.6 以配置条件化的直接回归骨干为对照表明：直接回归式架构在可再生份额谱上呈现 U 形迁移难度结构，而本方法的水平-形态分离结构性消除该结构，使迁移难度回归由 $L_T$ 支配、可部署前预测的形态。

**中国电网数据。**七个省级现货市场已正式运行（山西于 2023 年 12 月 22 日率先转正），并按 2024 年披露规则向市场成员发布 15 分钟级分电源数据；官方碳强度为滞后约两年的年度均值；月度发电统计完全公开 [12,14]。零遥测层恰好占据该制度形成的空档。本文中 ESPS 山西数据 [14] 仅用作评估侧真值，从不用作模型输入。
...
其中 $\mathcal{F} = \{\mathrm{pv}, \mathrm{wd}\} \cup \mathcal{B} \cup \mathcal{T}$，$|\mathcal{B}|{=}5$，$|\mathcal{T}|{=}3$，$EF_f$ 为固定常数。每个预测头编码一条物理机制假设。

**光伏头**——确定性天文通道叠加天气调制：

$$\hat s_{\mathrm{pv},t+h} = c^{\mathrm{m}}_{\mathrm{pv}} \cdot \frac{A_{t+h}}{\bar A_t} \cdot \bigl(1 + 0.4 \tanh g_\theta(w_{t+1:t+H})\bigr), \tag{4}$$

其中 $A$ 为晴空包络（纬度、日期与时刻的确定性函数），$c^{\mathrm{m}}$ 为月度配置份额，$\bar A_t$ 为窗口内均值。光伏出力的多数可预测结构来自天文周期，天气仅调制幅度。
...
其中 $\mathrm{wcf}(\cdot)$ 为由天气因果派生的容量因子曲线（24 h 均值与 6 h 趋势构成的运行工况特征），截断防止外推发散；风旱锚定参考在近期工况远低于年均水平时向年均回归。

**基荷头**——掩码水平统计：$\hat s_{b,t+h} = M_b \cdot \ell_b(c^{\mathrm{m}}, \mathrm{history})$，支撑掩码 $M_b = \mathbb{1}[c^{\mathrm{m}}_b > 0]$ 保证月度配置表中不存在的燃料在结构上不可能产生幻觉出力。
...
零初始化使新部署的模型由配置先验出发，将调度竞争作为残差学习。

**聚合头与结构路由。**对于燃料路径分解在结构上不可靠的配置——高风电占比区域（受风场代表性限制）与水电主导区域（可调度水电跟随负荷曲线，基荷头不可见）——聚合路径预测总可再生份额 $\hat r$（DLinear 骨干 [8]、logit 空间配置锚定、历史模式下持久性门控），并按 $\widehat{\mathrm{CIF}} = \hat r \cdot EF_\mathrm{ren} + (1-\hat r)\, EF_\mathrm{nr}$（其中 $EF_\mathrm{ren}{=}0$）重构。路由为配置的零参数确定性函数：
...
其中 $\tau = 1.1$（燃料路径优先；$c_\mathrm{wd} \ge \tau$ 时路由至聚合路径）；水电门 $c_\mathrm{hyd} \ge 0.5$ 具有经验支撑：在水电占比 71% 的 BPAT 区域上，聚合路径将冷启动模式 MAE 由 46.7 降至 15.2。路由决策在部署前即可审计，其仅读取公开配置数值。

### 4.3 训练与容量约束

式 (2) 的四项目标统一量纲，使梯度压力跟随误差的物理后果。消融实验表明配置距离加权采样为最强的单项训练组件（移除后性能恶化 18.5%）。冷启动 dropout 使同一套权重无需改动即同时服务于 $\mathcal{I}_\mathrm{cfg}$ 与 $\mathcal{I}_0$。全模型参数量约 2.1 万。三族深度条件化机制——特征调制（FiLM 式，约 30 倍参数量）、超网络尺度生成、元学习融合——在消融中一致降低零样本性能；其机制一致：模型容量记忆源域时序惯用模式，而惯用模式不可迁移。在零遥测预测中，模型简洁性是本文的一项发现而非局限。

### 4.4 校准层（$\mathcal{I}_+$）：免训练测试期校准
...
### 4.5 滞后层（$\mathcal{I}_\mathrm{lag}$）：免训练在线混合

三分量凸混合——EWMA 水平校正、日内锚定持久性、Chronos-2 [6] 形态先验：
...
**输入接口（全部公开、免注册）。**月度分燃料发电量表（省级统计公报，滞后约 1 个月）、省间月度受电表（电力交易中心）、ERA5 历史与日前天气 [13]、中国日历事实表（国务院节假日安排；供暖季规定：太原 11 月 1 日至 3 月 31 日，京津冀 11 月 15 日至 3 月 15 日）。

**翻译层（四组件，14 项单元测试）。**(i) 日历重映射：法定假日映射至周日通道，调休上班日映射至周一通道，模型经已学习的周末形态通道感知假日负荷形态；映射为建议性质，从不静默生效。(ii) 送端加权进口排放因子：进口电量采用送端省份月度结构隐含 CIF 的流加权平均（四川枯水期偏火电、丰水期偏水电），而非常数因子。(iii) 月度配置表：月度统计转换为 (12, 16) 配置契约，可再生部分按生态环境部口径核算，年度份额按电量加权。(iv) 代理锚定信任门控：无 CIF 观测的省份对照独立公开统计进行审计（火电电量指数与日电煤耗量的相关性，锚定官方年度因子）；无任何代理可用时门控关闭，系统回退至年度配置，按构造保守。

**独立真值验证。**山西对照 ESPS 公开数据集 [14]（35,136 行 15 分钟市场数据，2024-01-01 至 2025-04-07，严格仅作评估真值，从不用作输入）；上海对照公开月度统计与官方年度因子；日均参考取自 Carbon Monitor [12]。在严格事前发布日历下（每个预测月仅使用预测起点之前已发布的统计）：
...
**数据。**三个辖区的 29 个区域，全部为真实逐时数据，任何报告指标均不含合成序列：4 个澳大利亚 NEM 区域（AEMO 2023，NEMED DUID 燃料遥测）、17 个英国 DNO 区域（National Grid ESO Carbon Intensity API）、8 个美国平衡区（EIA-930）。天气数据为 Open-Meteo ERA5 再分析 [13]，对已知场群所在的 12 个区域做场站加权混合，其余 31 个覆盖点使用质心网格。中国侧设置见 §6。

**基线。**PatchTST + RevIN [7,9]（300 epochs，本地训练）、忠实复现的 CarbonCast [1]（相同测试窗口）、持久性基线（lag-24 h）、以及配置条件化的直接回归骨干 AdaptivePersistDLinear（§8.6 消融对照）。监督基线在相同测试窗口上训练（2 个随机种子，种子间差异 < 0.2 gCO₂/kWh）。
...
表 1 可从三个层面解读。(i) 零遥测层 38.4 低于两个监督参考：零遥测与 10 个月监督的对比在总体层面发生反转，18/29 区域直接占优。(ii) 遥测层 36.9 在 17/29 区域占优。(iii) 免训练滞后混合仅凭滞后观测即达到监督精度的 64%。校准层的评估目标是其设计目标本身——免训练恢复迁移损失——并在 25/29 区域优于持久性基线；将其与监督基线作绝对中位数比较会混淆协议口径，本文不作该比较。
...
**表 2.** 留一辖区实验：训练源仅为 US+UK；目标为全部四个澳大利亚电网。5 个随机种子的中位数；监督基线为相同测试窗口重训的 PatchTST。
...
**定义（无需市场数据即可复现）。**对每个区域与每个年份：设 $q^{w}_{20}, q^{s}_{20}$ 分别为 ERA5 [13] 日均 100 m 风速与地表短波辐射的年 20 分位数。日均风速低于 $q^{w}_{20}$ 的日期记为低风日（wind-lull），日均短波辐射低于 $q^{s}_{20}$ 的日期记为低辐照日（solar-dim），二者同时满足记为复合事件日（Dunkelflaute）；太阳阈值退化的暗冬区域仅评估低风日。标签纯由天气派生，任何团队均可再生成。预测误差按标签分桶，并以置换检验比较（29 区域 × 4 方法 = 348 项检验）。
...
**数据工程主导。**全部消融中幅度最大的三项改进均非架构性：统一 UTC 时基的输入管道（时区错位配置下天文通道与光伏份额相关系数为 −0.697，统一时基后为 +0.976，校准层连带受益）；场站加权天气（质心网格天气下风电份额解释方差 R² 为 0.08–0.15，场站加权后为 0.33–0.48）；结构路由阈值取 $\tau = 1.1$（相比更保守的 0.25，该取值将燃料路径覆盖扩展至中等风电占比区域，并降低高风电配置的误路由）。
...
**直接回归对照与迁移难度结构。**为分离物理分解架构的贡献，本文以配置条件化的直接回归骨干 AdaptivePersistDLinear（DLinear 骨干直接回归 CIF，配置经条件化层注入）为对照，在相同协议下评估。对照骨干的零样本迁移难度与可再生份额呈 U 形关系（二次拟合 R² = 0.662，顶点 0.58）：化石主导的配置谱边界区域（US_FPL/PJM/ISNE，效率比 ρ ≥ 2.6）与可再生能源极端区域均系统性失败。本方法的结构性改进正在于此：燃料分解骨干上该 U 形关系不再成立（R² = 0.087，顶点落于观测范围之外），边界区域的失败模式被水平-形态分离消除。迁移难度转而由定理 1 的 $L_T$ 排序刻画，可在部署前预测。此为 n = 29 的实证结论。

### 8.7 滞后层与 20 gCO₂/kWh 精度下界的不可达性

免训练滞后混合：EWMA 单独为 30.76（后期时段），引入 Chronos-2 后为 28.27，三分量混合为 **27.95**（5 个随机种子 27.97 ± 0.10；最难的九个区域由 47.58 降至 43.57）。残差分解表明：EWMA 水平误差为 21.0，而水平替换实验——保留 EWMA 形态、代入真实水平——给出的误差下界为 23.1。任何仅校正水平的零遥测方法因此不可能低于 23.1；在无新形态信息时，20 gCO₂/kWh 的零遥测目标不可达，而 Chronos-2 已吸收大部分可用形态先验。本文将该结论报告为对方法类的界，而非仅对本文实现的界。
...
6. Ansari, A.F., Schchur, O., Küken, J., et al., 2025. Chronos-2: from univariate to universal forecasting. arXiv:2510.15821.
