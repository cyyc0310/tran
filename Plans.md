...
| FD-41 | 训练时长臂:单 seed-0 全量扫描 epochs 900(22/29,p=0.020 显著)/ p_cold 0.4(拒绝 p=0.62)→ 900ep 全量 29×5 验证 | **采纳为新官方 fd41:I_cfg 38.29 / I_0 37.19 / I_+ 46.94**;fd41 vs fd39e 配对 I_cfg 97/145(p=0.001);代价=脆弱三区(UK_05 40.3→52.3/VIC1 81.3→84.5/BPAT 17.5→21.1),后续=按配置相似度的训练预算分配。集成叠加(−1.2)预计同向留复验。`fuel_decomp_eval_full_fd41.json`;`probe_hparam_arm.py` | FD-40 | `cc:完了` |
| FD-42 | 集成×900ep 叠加验证(10 代表区×5 seed×900ep) | **10/10 全正,均值 +1.03**:VIC1 81.4、UK_05 50.4、NYIS 19.6(<20);SA1 +0.1(水平偏置,集成无效,三次交叉确认)。部署栈定案:900ep×5-seed 集成×全燃料×合成份额省路由保护。`fd42_ensemble_900ep.json` | FD-41 | `cc:完了` |
| FD-43 | NWP 集合离散度通道(用户立项):Open-Meteo historical-forecast 真实归档预报(GFS+ICON,29 区×2023 全年 curl 提取),分歧通道 n_exog 18→20,env 门控 | 29×2 全量配对:**I_cfg 33/58(+0.42,p=0.40 n.s.)/ I_0 35/58(+0.79,p=0.11)**;辖区模式:UK 家族 10/13 正(UK_05 +19.7 两 seed 一致,I_0 63.7→40.3),US 多负 → 不进官方,归档。后续:spread 软门控 / 辖区可预测性先验。`fd43_nwp_full.json`;`fd43_nwp_verdict.md`;`download_nwp_forecasts.py` | FD-42 | `cc:完了` |
| FD-43b | 池化判决:补跑 seeds 2/3/4(87 对)凑满 145 对;软门控(幅度阈值)原理否决(UK/US 分歧分布一致) | **I_cfg 80/145(+0.67,p=0.102)——差 0.05 线一步,维持归档**;辖区分解 5-seed 稳定:UK +1.09/对、AU +0.49、US ≈0;UK_05 五 seed +55.7 全同向、VIC1 +16.6。后续:提功效/预注册交互项/UK 部署形态直用。`fd43_nwp_full_s234.json` | FD-43 | `cc:完了` |
| FD-44 | 预注册交互项(用户选):spread 通道 x 区域风电份额(FPL 0→精确归零),决策规则三项预注册 | **三项全 FAIL 拒绝**:77/145(p=0.249)/均值 +0.47(<0.9)/QLD1 -10.1;VIC1 +16.6→-5.0、SA1 -0.2→-6.0 反转(缩放杀收益),UK_05 不动。预注册流程防住事后包装。两版对照 `fd43_nwp_full.json`/`fd44_probe.json` | FD-43b | `cc:完了` |
